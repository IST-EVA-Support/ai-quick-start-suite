...
if [ -f "/opt/adlink/eva/plugins/libopcheck.so" ]
then
    message_out "/opt/adlink/eva/plugins/libopcheck.so exist, will remove it first."
    sudo rm /opt/adlink/eva/plugins/libopcheck.so
    message_out "sudo rm /opt/adlink/eva/plugins/libopcheck.so is removed."
else
    message_out "/opt/adlink/eva/plugins/libopcheck.so does not exist, will copy it"
fi

# copy built library to eva plugins folder
message_out "start copying to /opt/adlink/eva/plugins/ ......"
sudo cp ./build/libopcheck.so /opt/adlink/eva/plugins/.
...
message_out "Build plugin, opcheck, process completed!"
